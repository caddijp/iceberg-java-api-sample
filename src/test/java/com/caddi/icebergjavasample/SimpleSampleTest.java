...
public class SimpleSampleTest {
    ...
    void testDataInsert() throws Exception {
        ...
        var namespace = "test" + rnd.nextInt(1000);
        var tableName = "table" + rnd.nextInt(1000);
        System.out.println("test table " + namespace + "." + tableName);
        var target = new SimpleSample();
        var input = getClass().getResourceAsStream("/sample.jsonl");
        target.perform("http://localhost:8181", namespace, tableName, input);
